...
\excludeversion{CategoryTheory}
\includeversion{Model Categories}
\excludeversion{Simplicial Categories}
...
\begin{shaded}
%In summary, we can form the following commuting diagram, in which:
%the \!$\xymatrix{\ar@{->}[r]&}$\! are data, or easy to define immediately; the \!$\xymatrix{\ar@{~>}[r]&}$\! are to be factorised; the \!$\xymatrix{\ar@{..>}[r]&}$\! comes from the extension property that $X$, being fibrant, has with respect to acyclic cofibrations.
%\[\xymatrix{
%&A\coprod A\ar[r]^{(i,i)}\xycof[d]\ar@{~>}[ld]&B\coprod B\xycof[d]\ar[rd]^{(f,g)}\\
%A&CA\xyfib[l]_\sim\ar[r]&CA{\displaystyle\coprod_{\textup{\tiny$A\coprod A$}}}(B\coprod B)\ar[r]\xycof[d]\ar@{~>}[ld]&X\xyfib[d]\\
%&B&CB\xyfib[l]_\sim\ar[r]\ar@{..>}[ur]&\emptyset
%}\]
...
\end{proof}
%\[\xymatrix{
%&A\coprod A\ar[r]^{(i,i)}\xycof[d]&B\coprod B\xycof[d]\\
%A\ar[d]^i\ar[r]^{\imath_2}&CA\ar[r]\ar[d]&
%CA{\displaystyle\coprod_{\textup{\tiny$A\coprod A$}}}
%(B\coprod B)\xycof[d]&\\
%B\ar[r]&CA\coprod_AB\xycof[r]^\sim&CB&
%}\]
%
%
%\[\xymatrix{
